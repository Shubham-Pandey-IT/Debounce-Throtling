...
const EmployeeDetail = ({ employees }) => {
  ...
  if (!emp)
    return (
      <div className="text-white p-20 text-center uppercase tracking-widest opacity-30">
        Loading Profile...
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white p-6 md:p-12 font-[Sora,sans-serif]">
      <button
        onClick={() => navigate("/")}
        className="mb-10 text-white/40 hover:text-white transition-all flex items-center gap-2"
      >
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="bg-white/5 border border-white/10 p-10 rounded-[2.5rem] backdrop-blur-md">
          ...
          <p className="text-violet-400 font-bold mb-8 uppercase tracking-widest text-sm">
            {emp.company.title}
          </p>
        ...
        {/* Map Section */}
        <div className="rounded-[2.5rem] p-4 overflow-hidden border border-white/10 h-125 shadow-2xl bg-white">
          <iframe
            title="map"
            width="100%"
            height="100%"
